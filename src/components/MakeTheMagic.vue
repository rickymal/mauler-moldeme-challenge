...
const performController = new PerformController(moldemeService, aiApiService, route.query.auth as string, { on_data_performing, on_data_performed, redirectPage })
...
<template>
  <form @submit="submitForm" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
    <div class="mb-4">
      ...
      <input v-model="training_time"
        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
        id="training_time" type="text" name="uname" placeholder="">
    </div>
    <div class="mb-6">
      <label class="block text-gray-700 text-sm font-bold mb-2" for="password">
        Quantidade máxima de gerações
      </label>
      <input v-model="iteration_time"
        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 mb-3 leading-tight focus:outline-none focus:shadow-outline"
        id="iteration_time" type="text" name="psw" placeholder="">
    </div>
    <button @click="make_perform" class="bg-blue-500 text-white py-2 px-2 rounded ml-2 mr-2 mu-5 md-2"
